...
import { directoryMerchants, parseSectionGroup } from "./merchantDirectory"
...
const styleFiveCategoryGroups = [
  { title: "Coaching & sports", group: "Coaching" },
  { title: "Education", group: "Education" },
  { title: "Pet care", group: "Pet care" },
  { title: "Beauty & wellness", group: "Beauty & wellness" },
  { title: "Health", group: "Health" },
]

const styleFiveDirectorySections = styleFiveCategoryGroups.map(
  ({ title, group }) => ({
    title,
    services: directoryMerchants
      .filter((merchant) => merchant.group === group)
      .map((merchant): LifestyleService => ({
        name: merchant.name,
        provider: merchant.provider,
        category: merchant.category,
        distance: `${merchant.distance} km`,
        offer: merchant.offer,
        price: `From $${merchant.price} / ${merchant.unit}`,
        description: merchant.offer,
        image: merchant.image,
        badge: merchant.badge,
        likes: merchant.likes,
        clicks: merchant.likes,
    ...
    () =>
      [
        ...merchantSections
          ...
          .map((section) => ({ ...section, useAllServiceCard: false })),
        ...styleFiveDirectorySections.map((section) => ({
          ...section,
          useAllServiceCard: true,
        })),
      ].map((section) => ({
          ...section,
        ...
        >
          <div
            ...
            onPointerMove={(event) => {
              ...
              if (!pointer || pointer.id !== event.pointerId) return
              const distance = event.clientX - pointer.startX
              if (!pointer.didDrag && Math.abs(distance) < 8) return
              if (!pointer.didDrag) {
          ...
          >
            <div className="lifestyle-style-five__category-grid">
              {visibleCategories.map((category) => {
                const categoryClass = `lifestyle-style-five__category${
                  category.isComingSoon ? " is-coming-soon" : ""
                }`

                if (category.label === "Coaching") {
                  return (
                    <button
                      type="button"
                      className={`${categoryClass} is-link`}
                      key={category.label}
                      aria-label="Browse coaching and sports services"
                      onClick={() => navigate("/coaching?section=Coaching")}
                    >
            ...
            return (
              ...
              >
                <div className="lifestyle-style-five__heading">
                  ...
                  {!isPromotionalSection && (
                    <button
                      type="button"
                      onClick={() =>
                        navigate(
                          `/coaching?section=${encodeURIComponent(
                            parseSectionGroup(section.title),
                          )}`,
                        )
                      }
                    >
