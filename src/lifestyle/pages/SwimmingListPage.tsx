...
const searchSuggestions = ["Swimming", "Yoga", "Facial treatment", "Personal training"]
const sportCategories = ["All", "Swimming", "PT", "Tennis", "Yoga", "Basketball", "Soccer"]
...
const services = [
  { id: "aquakids", name: "AquaKids Swimming", provider: "AquaKids Academy", category: "Swimming", type: "Kids & beginner", distance: 1.2, price: 45, saves: 482, clicks: 1180, image: "https://images.unsplash.com/photo-1562271613-c3a4be2cc883?auto=format&fit=crop&w=760&h=460&q=85", tag: "Featured" },
  { id: "bluewave", name: "Bluewave Swim Academy", provider: "Bluewave Coaching", category: "Swimming", type: "Adult freestyle", distance: 2.4, price: 58, saves: 318, clicks: 940, image: "https://images.unsplash.com/photo-1530549387789-4c1017266635?auto=format&fit=crop&w=760&h=460&q=85", tag: "Popular" },
  { id: "tide", name: "Tide Swim Studio", provider: "Tide Sports", category: "Swimming", type: "Private coaching", distance: 3.1, price: 72, saves: 207, clicks: 770, image: "https://images.unsplash.com/photo-1530631673369-bc20fdb32288?auto=format&fit=crop&w=760&h=460&q=85", tag: "1-on-1" },
  { id: "lane", name: "Lane Eight Aquatics", provider: "Lane Eight", category: "Swimming", type: "Technique clinic", distance: 4.0, price: 48, saves: 164, clicks: 612, image: "https://images.unsplash.com/photo-1461567933755-6c82be2197da?auto=format&fit=crop&w=760&h=460&q=85", tag: "New" },
]
...
export default function SwimmingListPage() {
  ...
  return (
    <main className="service-page">
      <StyleTwoPageHeader title="Coaching & sports" onBack={() => navigate(-1)} />
      <section className="service-content">
        <label className="service-search"><span className="service-search-icon" aria-hidden="true"><SearchIcon /></span><input value={query} onChange={(event) => setQuery(event.target.value)} aria-label="Search swimming services" />{!query && <span className="service-search-placeholder" aria-hidden="true"><span>Search&nbsp;</span><span className={`service-search-term${searchSuggestionVisible ? " is-visible" : ""}`}>{`\u201c${searchSuggestions[searchSuggestionIndex]}\u201d`}</span></span>}{query && <button type="button" className="service-search-clear" onClick={(event) => { event.preventDefault(); setQuery("") }} aria-label="Clear search"><svg viewBox="0 0 24 24"><path d="m7 7 10 10M17 7 7 17" /></svg></button>}</label>
        <div ref={filterRowRef} className={`filter-row${filterDragging ? " is-dragging" : ""}`} role="tablist" aria-label="Coaching and sports categories" onPointerDown={startFilterDrag} onPointerMove={moveFilterDrag} onPointerUp={finishFilterDrag} onPointerCancel={finishFilterDrag} onClickCapture={(event) => { if (!suppressFilterClickRef.current) return; event.preventDefault(); event.stopPropagation(); suppressFilterClickRef.current = false }}>{sportCategories.map((item) => <button type="button" key={item} className={filter === item ? "active" : ""} role="tab" aria-selected={filter === item} aria-pressed={filter === item} onClick={(event) => selectFilter(item, event.currentTarget)}>{item}</button>)}</div>
        ...
        <div className="service-list">{filtered.map((service) => { const isSaved = saved.includes(service.id); return <article className="service-card" key={service.id} onClick={() => navigate(`/swimming/${service.id}`)}><div className="service-card-image"><img src={service.image} alt={service.name} /><span>{service.tag}</span></div><div className="service-card-body"><p className="service-provider">{service.provider}</p><h2>{service.name}</h2><p>{service.type}</p><div><HugeiconsIcon icon={Location01Icon} size={13} color="#9aa6b2" strokeWidth={1.8} /><span>{service.distance} km away</span></div><span className="service-price"><span className="service-price-from">From </span><strong className="service-price-value">${service.price}</strong><span className="service-price-unit"> / session</span></span></div><button className={isSaved ? "favorite-button saved" : "favorite-button"} onClick={(event) => { event.stopPropagation(); toggleSaved(service.id) }} aria-label={isSaved ? "Remove from saved" : "Save service"}><HugeiconsIcon className={isSaved ? "icon-filled" : ""} icon={HeartIcon} size={14} color="currentColor" /><small>{service.saves + (isSaved ? 1 : 0)}</small></button></article> })}</div>
      </section>
    </main>
  )
}
